...
struct fat_directory_item {
    uint8_t filename[8];                  // 文件名（8字节）
    uint8_t ext[3];                       // 文件扩展名（3字节）
    uint8_t attribute;                    // 文件属性（1字节）
    uint8_t reserved;                     // 保留字段（1字节）
    uint8_t creation_time_tenths_of_a_sec; // 创建时间的百分之一秒（1字节）
    uint16_t creation_time;               // 创建时间（2字节）
    uint16_t creation_date;               // 创建日期（2字节）
    uint16_t last_access;                 // 最后访问日期（2字节）
    uint16_t high_16_bits_first_cluster;  // 第一个簇的高16位（2字节）
    uint16_t last_mod_time;               // 最后修改时间（2字节）
    uint16_t last_mod_date;               // 最后修改日期（2字节）
    uint16_t low_16_bits_first_cluster;   // 第一个簇的低16位（2字节）
...
int fat16_get_root_directory(struct disk *disk, struct fat_private *fat_private, struct fat_directory *directory)
{
    ...
    int total_sectors = root_dir_size / disk->sector_size;//根目录扇区数
